const getContactsBaltic = () => {
  ...
  let options = {
    ...
    let url = "https://api.infusionsoft.com/crm/rest/v1/contacts/?limit=1000&offset="+offset+"&order=id&optional_properties=custom_fields";
      ...
      contactsDetails = contactsDetails.concat(contactsArray(data.contacts,"Baltic",customFieldsBaltic));
      offset = offset+1000;
...
const customFieldsBaltic = {
    refferal_Group : 23,
    nurturing_Group : 57,
    membershipType: 27,
    membership_Commencement_Date : 25,
    membership_Cancellation_Date : 49,
    date_of_Final_Payment : 39
}

const filteredContactsBaltic = () => {
let contactsList = [];

let sheet = taggedContactsSheet.getRange("A2:E").getValues();
sheet = sheet.filter(element => element[0] != ""); //removeEmpty values
sheet = sheet.filter(element => element[4] == "Baltic");
...
contacts = contacts.filter(contact => contact[18] == "Baltic")
